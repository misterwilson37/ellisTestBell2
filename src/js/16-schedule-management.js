...
import { findPeriodEdgeAnchorBell, formatTime12Hour, timeToSeconds } from './00-header.js';
import {
...
import {
    generateBellId, generatePeriodId, normalizeTimeString, updateCalculatedTime,
...
function openEditRelativeModal(bell) {
    ...
    if (rawBell.relative.parentBellId) {
        ...
        // V6.8.0: routed through the engine's findPeriodEdgeAnchorBell (one
        // implementation, one behavior — this inline copy had drifted: with
        // only a wrong-edge anchorRole bell present it selected THAT bell for
        // the opposite edge; the primitive correctly returns null and we
        // leave the select blank for the user, matching resolution behavior).
        const edgeBell = findPeriodEdgeAnchorBell(targetPeriod, rawBell.relative.parentAnchorType);
        if (edgeBell) {
            relativeAnchorBellSelect.value = edgeBell.bellId;
        } else {
            // No anchor bell found - leave blank
            relativeAnchorBellSelect.value = '';
    ...
    if (bell.isRelative && bell.type === 'custom') {
        ...
        if (bell.type === 'shared') {
            ...
            if (isAdmin) {
                ...
                // fire-and-forget by design — see 30-building-bells.js)
...
async function handleEditBellSubmit(e) {
    ...
    let anchorNote = ''; // V6.5.0: set by the Building Bells anchor resolution below
    ...
    try {
        ...
        if (oldBell.type === 'shared') {
            ...
            // V6.5.0 (Building Bells): apply the anchor select before the periods
            // rewrite. resolveEditBellAnchorForSave returns {present:false} when
            // the select was never shown (then buildingBellId stays untouched and
            // updatePeriodsOnEdit preserves any existing anchor); otherwise
            // {id: string|null, mismatchName?} — null clears the anchor, and a
            // selection whose building-bell time no longer matches the typed time
            // is DETACHED (an anchor means "this IS that building moment"; keeping
            // it with a divergent time would let the next building-bell edit snap
            // this bell back unexpectedly).
            const anchorSel = resolveEditBellAnchorForSave(newBell.time);
            if (anchorSel.present) {
                newBell.buildingBellId = anchorSel.id;
                if (anchorSel.mismatchName) {
                    anchorNote = ` Building-bell anchor "${anchorSel.mismatchName}" was removed — times no longer match.`;
                }
            }
            ...
            logScheduleEdit(state.activeBaseScheduleId, 'edit-bell', { // V5.75.0
                ...
                after: { name: newBell.name, time: newBell.time, sound: newBell.sound,
                         buildingBellId: newBell.buildingBellId || null } // V6.5.0
            });
...
function updatePeriodsOnEdit(periods, oldBell, newBell) {
    ...
    const updatedPeriods = periods.map(period => {
        ...
        if (bellIndex > -1) {
            ...
            let updatedBell = originalAnchorRole 
                ? { ...newBell, anchorRole: originalAnchorRole }
                : { ...newBell };
            // V6.5.0 (Building Bells): preserve the additive buildingBellId anchor
            // unless the edit EXPLICITLY sets it (string) or clears it (null).
            // Personal-bell callers never carry the property, so their edits pass
            // through without touching anchors. Without this, every edit through
            // this function would silently strip the anchor — the exact

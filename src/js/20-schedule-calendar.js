...
import { doc, onSnapshot, getDoc, setDoc } from './01-firebase-imports.js';
...
function refreshActiveTransforms() {
    ...
    publishClockFeeds(); // v6.19.0: keep the dumb-clock feed current (admin only)
}

// ===== V6.19.0: wall-clock feed =====
// Dumb hallway TVs/clocks (old.html, unauthenticated ES5) can't do recipe
// math, so an authenticated ADMIN precomputes each targeted schedule's
// TRANSFORMED periods for today and publishes them, flat, to the public
// config/clock_feeds doc. A transform entry opts in via clockScheduleIds
// (the "Show on clocks" picker). config/{id} is already public-read +
// admin-write (firestore.rules), so no rules change. Feeds are dated; a stale
// one is ignored by the clock. Runs on every calendar/schedule/day trigger via
// refreshActiveTransforms; writes only succeed for admins (rules), so it
// no-ops for everyone else.
function clockFeedsRef() {
    return doc(state.db, 'artifacts', state.appId, 'public', 'data', 'config', 'clock_feeds');
}
function publishClockFeeds() {
    if (!state.isAdmin || !state.db || !scheduleCalendar) return;
    const today = toLocalDateString(new Date());
    const day = scheduleCalendar.days && scheduleCalendar.days[today];
    const entries = (day && Array.isArray(day.entries)) ? day.entries : [];

    // schedule id -> ordered recipes that target its clocks today
    const bySchedule = {};
    entries.forEach((e) => {
        if (e && e.verb === 'transform' && e.recipe && Array.isArray(e.clockScheduleIds)) {
            e.clockScheduleIds.forEach((sid) => {
                (bySchedule[sid] = bySchedule[sid] || []).push(e.recipe);
            });
        }
    });

    const buildFeed = (sid, recipes) => {
        const sched = (state.allSchedules || []).find((s) => s.id === sid);
        if (!sched || !Array.isArray(sched.periods)) return null;
        let periods = sched.periods;
        recipes.forEach((r) => { periods = window.BellEngine.applyRecipeToPeriods(periods, r).periods; });
        return { date: today, periods: periods, scheduleName: sched.name, updatedAt: Date.now() };
    };

    const feeds = {};
    Object.keys(bySchedule).forEach((sid) => {
        const f = buildFeed(sid, bySchedule[sid]);
        if (f) feeds[sid] = f;
    });

    // Reset any schedule we published for EARLIER TODAY that's no longer
    // targeted (e.g. the admin removed the transform mid-day) back to its base,
    // so the clock stops showing a since-cancelled change before midnight.
    getDoc(clockFeedsRef()).then((snap) => {
        const existing = (snap.exists() && snap.data() && snap.data().feeds) || {};
        Object.keys(existing).forEach((sid) => {
            if (existing[sid] && existing[sid].date === today && !bySchedule[sid]) {
                const base = buildFeed(sid, []); // base periods, dated today
                if (base) { base.reset = true; feeds[sid] = base; }
            }
        });
        if (Object.keys(feeds).length) {
            setDoc(clockFeedsRef(), { feeds: feeds }, { merge: true })
                .catch((e) => console.warn('[ClockFeed] publish failed:', e && e.message));
        }
    }).catch(() => {
        // couldn't read existing feeds; still publish what we computed
        if (Object.keys(feeds).length) {
            setDoc(clockFeedsRef(), { feeds: feeds }, { merge: true })
                .catch((e) => console.warn('[ClockFeed] publish failed:', e && e.message));
        }

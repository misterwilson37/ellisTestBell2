...
let currentCustomBellIconSlot = null;
...
let currentSoundSelectTarget = null;
...
(function() {
    ...
    function updateClock() {
        ...
        if (millisToScheduleBell < millisToQuickBell && scheduleBellObject) {
            ...
            activeTimerLabel = quickBellEndTime.bellName || "Quick Bell";
    ...
    */
        ...
        personalBellsPeriods.forEach(period => {
            if (mergedPeriodsMap.has(period.name)) {
                ...
                mergedPeriodsMap.set(period.name, {
                    ...
                    origin: 'personal'  Mark as fully custom
                });
            }
        });
    ...
    function closeAllConflictModals() {
     ...
     Function to add the pending custom bell (Req 1)
    ...
    async function initFirebase() {
        ...
        try {
            // The dynamic __firebase_config check is ONLY for the
            ...
            */
             ...
             onAuthStateChanged(auth, async (user) => {
                ...
                if (user) {
                    ...
                    userId = user.uid;
                    isUserAnonymous = user.isAnonymous;  Track anonymous state
                    welcomeOverlay.classList.add('hidden');

                     Handle visibility based on anonymous state
                    if (user.isAnonymous) {
                    ...
                    if (!isAudioReady) {
                        console.log("User is signed in, but audio not ready. Showing audio start button.");
                         ...
                         Set custom button text based on user
                    ...
                    if (user.isAnonymous) {
                       ...
                       userDisplayNameElement.textContent = "Signed in as: Anonymous";
                        ...
                        Display User ID in the footer, not the header
                    ...
                    if (!user.isAnonymous && user.uid) { // Check for non-anonymous user
                     ...
                     Enable admin toggle *only* if server-side check passed
                    if (isAdmin) {
                    ...
                    await listenForSharedSchedules();
                     ...
                     Load audio files if not anonymous
                    if (!user.isAnonymous) {
                    ...
                    userDisplayNameElement.textContent = "";  Clear header display name
                    adminToggleBtn.disabled = true;
                    adminToggleBtn.classList.add('opacity-50', 'cursor-not-allowed');
        ...
        if (type === 'shared') {
            ...
            activePersonalScheduleListenerUnsubscribe = onSnapshot(personalScheduleRef, (docSnap) => {
                ...
                if (docSnap.exists()) {
                    ...
                    let periodsToUse = [];
                    let fieldsToUpdate = {};  Track updates
    ...
    async function loadAllVisualFiles() {
        ...
        if (!isUserAnonymous && userId) {
            ...
            try {
                ...
                userVisualFiles = await Promise.all(userFilesResult.items.map(async (itemRef) => {
                    ...
                    const nickname = meta.customMetadata?.nickname || '';
                    return { name: itemRef.name, url: url, path: itemRef.fullPath, nickname: nickname };
                }));
        ...
        try {
            ...
            sharedVisualFiles = await Promise.all(publicFilesResult.items.map(async (itemRef) => {
                ...
                const nickname = meta.customMetadata?.nickname || '';
                return { name: itemRef.name, url: url, path: itemRef.fullPath, owner: owner, nickname: nickname };
            }));
   ...
   async function signOutUser() {
    ...
    function toggleAdminMode() {
        ...
        if (document.body.classList.contains('admin-mode')) {
         ...
         Re-render audio list to show/hide admin controls
        renderAudioFileManager();
        renderVisualFileManager(); // NEW in 4.44
        renderCombinedList();  Re-render list to show/hide admin buttons
        setActiveSchedule(scheduleSelector.value);
    }
    ...
    async function handleAudioUpload() {
        ...
        try {
            ...
            const newFile = { name: fileToUpload.name, url: downloadURL, path: storageRef.fullPath, nickname: '' };
            userAudioFiles.push(newFile);
            userAudioFiles.sort((a, b) => (a.nickname || a.name).localeCompare(b.nickname || b.name));
            addNewAudioOption(downloadURL, fileToUpload.name, '');
     ...
     */
    function addNewAudioOption(url, name, nickname = '') {
        const selects = [
        ...
        selects.forEach(select => {
            ...
            if (optgroup) {
                const option = document.createElement('option');
                option.value = url;
                option.textContent = nickname || name;
                optgroup.appendChild(option);
            }
        });
    }
    ...
    function updateSoundDropdowns() {
     ...
     Helper function to find all bells using a specific sound URL
    function findBellsUsingSound(url) {
     ...
     Helper function to update all bells using a specific sound URL
    ...
    async function handleAudioListClick(e) {
        ...
        if (target.classList.contains('delete-audio-btn')) {
            ...
            const url = target.dataset.url;  Get URL
            if (!path || !url) return;
         ...
         Handle Make Public (Admin) button click
     ...
     Function to execute the audio deletion
    ...
    function init() {
        ...
        customQuickBellListContainer.addEventListener('click', (e) => {
            ...
            const iconBtn = e.target.closest('.custom-bell-icon-btn');
        ...
        customQuickBellListContainer.addEventListener('change', (e) => {
         ...
         Quick Launch Listener for Custom Buttons
        ...
        deleteAudioCancelBtn.addEventListener('click', () => {
         ...
         Modals (Nearby Bell - Custom Only)
       ...
       multiSelectNoneBtn.addEventListener('click', () => {
        ...
        combinedBellListElement.addEventListener('click', handleBellListClick);
         ...
         Separate change handler for mute toggles (checkboxes respond better to 'change' than 'click')
            ...
            const updatedPeriods = existingPeriods.map(period => {
                ...
                const newBells = period.bells.map(bell => {
                    if (childMap.has(bell.bellId)) {
                        ...
                        // const childBell = childMap.get(bell.bellId);  This is the raw bell
                        bellsChanged = true;
